...
export default function Showstaff() {
  ...
  async function datalao() {
    try {
      const response = await axios.get("http://localhost:3007/Web/staff");
  ...
  async function remove(id) {
    try {
      if (window.confirm("Are you sure you want to delete this record")) {
        await axios.delete(`http://localhost:3007/Web/staff/${id}`);
        datalao();
  ...
  async function Edit() {
    try {
      const res = await axios.put(`http://localhost:3007/Web/staff/${id}`, {
        staff_name: na,
        staff_email: em,
        Age: age,
        Gender:gen,
