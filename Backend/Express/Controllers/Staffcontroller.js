...
let main_function={
    ...
    staff_register:async function(req,res){
        try {
            ...
            let check_mail=await user.findOne({staff_email:staff_email})
            if(check_mail){
            return res.status(409).json({msg:"Email already exist"})
            }
            else{
                let getpassword=bcrypt.hashSync(password,12)
                let user_data= new user({staff_name,staff_email,Age,Gender,Phone_no,Address,Role,Shift_Timing,password:getpassword})
